#!/usr/bin/env bash
# Run picom against yserver as a RENDER-convolution smoke harness
# (phase 2 of `docs/superpowers/plans/2026-05-15-render-convolution-
# filter.md`).
#
# picom v13's `xrender` backend issues
# `XRenderSetPictureFilter "convolution"` for the configured
# `blur-kern` when it starts up, regardless of whether any window
# currently has blur enabled. That's sufficient to exercise yserver's
# convolution-pipeline code path: look for these markers in
# `yserver-hw.log`:
#
#   render_set_picture_filter: picture=… filter=convolution NxM
#   render_composite: convolution-bearing composite …
#   vk composite: convolution-source gating …
#   vk convolution composite: kernel out of range …  (failure mode)
#
# If `convolution-bearing composite` fires but the `gating` log shows
# a rejected predicate (e.g. `mask_none=false`), that pinpoints
# where the phase 2 dispatch falls through.
#
