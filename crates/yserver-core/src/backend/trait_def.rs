...
pub enum BackendFdKind {
    ...
    Libinput,
    /// DRM device fd; readiness drives `on_page_flip_ready` with this
    /// fd so a multi-device backend drains only the ready device.
    Drm,
    ...
    DrmHotplug,
    /// Backend-internal readiness aggregator for renderer-completion
    /// `sync_file` fds which gate a copied scanout path. Readiness means a
    /// frame rendered on the source GPU may advance to the sink-GPU copy.
...
pub trait Backend {
    ...
    /// `drm_fd` is readable. The backend should drain completion
    /// events from that exact DRM device and submit the next
    ...
    /// The core gives every entry a unique token and retains both the
    /// fd and its `BackendFdKind` for readiness dispatch.
