...
//!   and signalfd. These never change at runtime.
//! - Backend-owned fds, allocated densely from `0x100` in the order
//!   returned by `Backend::poll_fds`. Every fd gets its own token so
//!   readiness can be routed back to that exact source, even when a
//!   backend exposes multiple fds of the same kind (for example one
//!   DRM fd per KMS device).
...
/// First token usable for backend-owned poll sources. The core keeps
/// the corresponding `(fd, BackendFdKind)` records in the same order.
...
/// Map an index in the core's backend poll-source table to a unique
/// token. Returns `None` when the table would overlap client tokens.
#[must_use]
...
/// Inverse of [`backend_token`]. Tokens outside the backend-owned
/// range return `None`.
#[must_use]
